...
pub fn compute(
    ...
    let input_h = if compact { 1 } else { input_height.clamp(1, 10) };
    let footer_h: u16 = if compact { 0 } else { 1 };
    let status_h: u16 = if turn_running && !compact { status_indicator_lines.max(1) } else { 0 };
    ...
    let chat_view = Rect { x: area.x, y: area.y, width: area.width, height: chat_h };

    let mut y = area.y + chat_h;

    let status_indicator = if status_h > 0 {
        let r = Rect { x: area.x, y, width: area.width, height: status_h };
        y += status_h;
        Some(r)
    } else {
        None
    ...
    let pending_preview = if pending_h > 0 {
        let r = Rect { x: area.x, y, width: area.width, height: pending_h };
        y += pending_h;
        Some(r)
    } else {
        None
    ...
    let input_editor = Rect { x: area.x, y, width: area.width, height: input_h };
    y += input_h;

    let footer = if footer_h > 0 {
        Some(Rect { x: area.x, y, width: area.width, height: footer_h })
    } else {
        None
    };

    Zones { chat_view, status_indicator, pending_preview, input_editor, footer }
}
