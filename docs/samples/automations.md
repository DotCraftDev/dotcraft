...
- [example-local-task](https://github.com/DotHarness/dotcraft/tree/master/samples/automations/example-local-task)：**本地自动化任务** — 基于文件的任务，完全在本地运行，不依赖外部服务。
- [github-review-bot](https://github.com/DotHarness/dotcraft/tree/master/samples/automations/github-review-bot)：**PR 审查机器人** — 自动检出所有非草稿的开放 Pull Request，分析 diff 并提交结构化 `COMMENT` 审查。推送新提交后自动重新审查。
- [github-collab-dev-bot](https://github.com/DotHarness/dotcraft/tree/master/samples/automations/github-collab-dev-bot)：**协作开发机器人** — 针对 GitHub Issue 进行规划、实现并创建 PR，通过标签协调多次运行间的状态。
...
- 本机已安装或编译 DotCraft
- 已选定一个项目目录作为 DotCraft workspace（运行 `dotcraft gateway` 时的当前工作目录）
...
运行 `dotcraft gateway`。机器人会自动检出所有非草稿的开放 PR。
...
编辑 `.craft/config.json` 填入你的仓库和令牌，然后运行 `dotcraft gateway`。
