...
- `plugin/install`：将 DotCraft 管理的内置插件部署到当前工作区 `.craft/plugins/<id>/`，默认启用。
- `plugin/setEnabled`：只切换已安装插件是否进入 Agent 上下文，不安装也不删除目录。
- `plugin/remove`：只移除带 `.builtin` marker 的 DotCraft 管理内置插件目录；不会删除用户接管的本地插件目录。

插件安装、移除或启用状态变化会广播 `workspace/configChanged`，`regions: ["plugins", "skills"]`。插件贡献的 tools 在会话中投影为 `pluginFunctionCall` item；它们不会再生成 companion `toolCall` / `toolResult` item。完整字段和错误行为见 [AppServer Protocol Spec](https://github.com/DotHarness/dotcraft/blob/master/specs/appserver-protocol.md)，插件架构见 [Plugin Architecture Spec](https://github.com/DotHarness/dotcraft/blob/master/specs/plugin-architecture.md)。
