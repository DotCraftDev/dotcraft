...
| 场景 | 使用方式 |
|------|----------|
| 第一次确认模型是否能调用 | 启用 DashBoard 后触发一次 CLI 或 Desktop 会话，查看 Trace Timeline |
...
- DashBoard 会暴露 prompt、工具参数和工具结果，默认建议绑定 `127.0.0.1`。
- Gateway 模式下可与 API、AG-UI 共用端口。
- Settings 页面保存配置后，启动级字段仍可能需要重启 Host 才能生效。
...
Automations 面板只在 Gateway 加载 Automations 模块时可用。CLI 独立 DashBoard 不注册 Automations 快照。
