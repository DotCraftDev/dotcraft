...
需要同时运行 API、AG-UI、Automations 或外部渠道时，可以使用 Gateway：
...
Automations 面板需要 Gateway 加载 Automations 模块。CLI 独立 Dashboard 适合调试当前会话，不负责展示完整自动化编排状态。
