...
Automations 模块提供统一的自动化任务管线，支持本地任务和 GitHub Issue/PR 编排。本地任务仅需 `Automations` 模块；GitHub 来源需同时启用 `Automations` 和 `GitHubTracker`。

完整使用说明见 [Automations 指南](./automations_guide.md)。
...
API 模式通过 `AutoApprove` 控制敏感操作的审批行为：

- **`AutoApprove: true`**（默认）：所有文件操作和 Shell 命令自动批准
- **`AutoApprove: false`**：所有文件操作和 Shell 命令自动拒绝

详细说明见 [API 模式指南](./api_guide.md#操作审批)。
