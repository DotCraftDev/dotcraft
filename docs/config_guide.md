...
| `CompactSessions` | 保存会话时是否截断过长的工具结果（超过 500 字符） | `true` |
...
| `MaxContextTokens` | 触发自动上下文压缩的累积输入 Token 阈值。`0` 表示禁用 | `160000` |

DotCraft 自动从 LLM 响应中提取 token 用量信息并在 CLI/QQ 模式下显示。
...
| `Heartbeat.NotifyAdmin` | QQ 模式下是否将结果私信通知管理员 | `true` |
...
根级别 `EnabledTools` 字段控制全局可用工具（为空时启用所有工具）：`SpawnSubagent`、`ReadFile`、`WriteFile`、`EditFile`、`GrepFiles`、`FindFiles`、`Exec`、`WebSearch`、`WebFetch`、`Cron`、`WeComNotify`。
...
ACP 模式通过 stdin/stdout 以 JSON-RPC 2.0 协议通信，优先级高于 API 模式。在 Gateway 模式下可与 QQ Bot、WeCom Bot、API 并发运行。
...
Gateway 模式允许 QQ Bot、WeCom Bot、API、AG-UI、Automations 等多个服务在同一进程中并发运行。

**自动启用**：当配置中启用了任意 Channel（`QQBot`/`WeComBot`/`Api`/`AgUi`/`Automations`/`GitHubTracker` 等），Gateway 模式会自动激活，无需手动配置。
