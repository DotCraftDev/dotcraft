...
QQ 已由 TypeScript 外部渠道 `@dotcraft/channel-qq` 提供，旧的 `QQBot` 配置段不再生效。

详细说明请参考 [QQ 外部渠道使用指南](./qq_bot_guide.md)。工作区配置通过 `ExternalChannels` 注册渠道，OneBot 监听地址、权限白名单和审批超时等渠道专属设置放在 `.craft/qq.json`。
...
ACP 模式通过 stdin/stdout 以 JSON-RPC 2.0 协议通信，使用 `dotcraft acp` 显式启动。在 Gateway 模式下，QQ Bot、WeCom Bot、API、AG-UI、Automations 等服务可以并发运行。
