...
本文档是完整配置参考，介绍 DotCraft 的全局配置、工作区配置、安全设置、工具、入口和扩展能力。如果你是第一次使用，请先阅读 [快速开始](./getting-started.md)，跑通 Desktop、工作区和模型配置后再回到这里查字段。
...
通过 [OpenSandbox](https://github.com/alibaba/OpenSandbox) 将 Shell 和 File 工具执行环境放到隔离的 Docker 容器中。
...
| 场景 | 推荐配置入口 |
|------|--------------|
| 第一次本地使用 | [快速开始](./getting-started.md) 中的最小模型配置 |
| 桌面端可视化协作 | `ApiKey` / `Model` / `EndPoint` + [Desktop 指南](./desktop_guide.md) |
| 调试工具调用和 Trace | `DashBoard.Enabled = true` + [Dashboard 指南](./dash_board_guide.md) |
| 多入口并发运行 | `dotcraft gateway` + Gateway / API / AG-UI / 外部渠道配置 |
| 接入机器人或外部适配器 | `AppServer.Mode = WebSocket` + `ExternalChannels` |
| 限制敏感路径访问 | `Security.BlacklistedPaths` + `Tools.*.RequireApprovalOutsideWorkspace` |

## Advanced Topics

- AppServer、API、AG-UI、ACP 和 Dashboard 的字段只在对应入口启用时有意义。
- 启动级配置修改后通常需要重启 DotCraft Host 或 Desktop 后台 AppServer。
- 安全配置全局生效，CLI、Desktop、外部渠道、自动化任务都会遵循同一套边界。
- 工作区 `.craft/config.json` 不建议保存 API Key，除非该仓库不会被共享或提交。

## Troubleshooting

### 配置文件写了但没有生效

确认写入的是当前工作区的 `.craft/config.json`，并重启相关 Host。AppServer、端口、Gateway、外部渠道等启动级字段不会自动热更新。

### API Key 不想提交进仓库

把 `ApiKey` 放在全局 `~/.craft/config.json`，工作区配置只保留 `Model`、`EndPoint` 和功能开关。

### Dashboard 或 API 端口冲突

Gateway 模式下同一 `Host:Port` 会由 WebHostPool 合并；如果不是 Gateway，请为不同服务配置不同端口。

### 工具访问被拒绝

检查 `Security.BlacklistedPaths`、工作区边界和 `Tools.File` / `Tools.Shell` 审批策略。黑名单优先级高于工作区边界。
