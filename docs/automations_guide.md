...
> `Automations.Enabled` 用于启动编排器，且默认值为 `true`。`GitHubTracker.Enabled` 用于注册 GitHub 来源。只要 `GitHubTracker.Enabled` 为 `true`，且 `Automations.Enabled` 没有被显式设置为 `false`，GitHub 任务就会被调度。
>
