...
GitHub 来源需要同时启用 `Automations` 和 `GitHubTracker` 两个模块。
...
> `Automations.Enabled` 启动编排器。`GitHubTracker.Enabled` 注册 GitHub 来源。两者都为 `true` 时 GitHub 任务才会被调度。
>
