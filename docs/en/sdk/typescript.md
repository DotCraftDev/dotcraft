...
- Pass `debugStream: true` in [`ChannelAdapter`](https://github.com/DotHarness/dotcraft/blob/master/sdk/typescript/src/adapter.ts) options (6th constructor argument). Logs use the prefix `[dotcraft-wire:adapter-stream]`.
- Call `configureTextMergeDebug(true)` from [`turnReply.ts`](https://github.com/DotHarness/dotcraft/blob/master/sdk/typescript/src/turnReply.ts) for merge traces (`[dotcraft-wire:text-merge]`).
