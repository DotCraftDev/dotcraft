...
- [example-local-task](https://github.com/DotHarness/dotcraft/tree/master/samples/automations/example-local-task): A **local automation task** — file-based task that runs entirely on-disk without any external service.
- [github-review-bot](https://github.com/DotHarness/dotcraft/tree/master/samples/automations/github-review-bot): A **PR review bot** that automatically picks up open, non-draft pull requests, analyzes the diff, and submits a structured `COMMENT` review. Re-reviews automatically when new commits are pushed.
- [github-collab-dev-bot](https://github.com/DotHarness/dotcraft/tree/master/samples/automations/github-collab-dev-bot): A **collaborative development bot** that plans, implements, and opens a PR for a given GitHub issue, using labels to coordinate state across runs.
