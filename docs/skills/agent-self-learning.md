...
| `Skills.SelfLearning.AllowDelete` | `false` | 是否启用 `SkillManage(action: "delete")`。删除是破坏性操作，必须单独开启。 |
...
| `delete` | `name` | 删除整个工作区 skill，仅在 `AllowDelete=true` 时可用。 |
...
`SkillManage` 在执行 `create` 与 `delete` 前会触发 DotCraft 审批（`kind: skill`），与文件/Shell 审批一致；`edit` / `patch` / `write_file` / `remove_file` 不需要审批。
