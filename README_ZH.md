...
DotCraft 的 Automations 使用统一的 `AutomationOrchestrator` 编排自动化任务，当前包含 `Local` 与 `GitHub` 两类来源。启用 `Automations` 模块后即可运行本地任务；额外启用 `GitHubTracker` 模块后，会把 GitHub Issue / Pull Request 作为 `GitHubAutomationSource` 接入同一条自动化管线，由 AppServer 托管轮询、派发 Agent、维护审核状态，并在 Desktop 的 Automations 面板中与本地任务并列展示。详见 [Automations 指南](./docs/automations_guide.md)。
