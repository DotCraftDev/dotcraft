...
DotCraft 的优势不只是“多个入口共享同一个工作区”。它让 CLI、IDE、内建机器人和外部渠道适配器都连接到同一个会话核心，同时仍然保留各自入口原生的交互模型。

传统 Gateway 风格的渠道架构，往往会为了统一路由而把不同平台的事件压平成通用消息总线，但这也会削弱平台原生的交互能力。DotCraft 则把外部渠道视为完整的双向 Wire Protocol Client，因此审批、确认、回调按钮以及其他 HITL 交互，仍然可以在 Telegram、QQ、企业微信等平台上以原生方式呈现。
...
这类适配器既可以通过 stdio 以子进程方式运行，也可以通过 WebSocket 独立连接；只要能够实现 JSON-RPC，就可以使用任意语言开发。Python SDK 同时提供低层 `DotCraftClient` 与高层 `ChannelAdapter` 基类，便于处理线程管理、事件流、消息投递与审批流程。在这个模型里，适配器本身就是完整的 Wire Protocol Client：DotCraft 可以将审批请求反向发回适配器，而适配器再决定如何用对应平台的原生 UI 呈现审批体验，而不是被迫退化成统一的 Gateway 交互层。

仓库中已经包含一个 Telegram 参考适配器，演示了 long polling、Inline Keyboard 审批以及与 DotCraft 会话模型的完整闭环。可进一步查看 [Python SDK](./sdk/python/README.md) 和 [外部渠道适配器规范](./specs/external-channel-adapter.md)。
