...
- ⚡ **统一会话核心**：统一所有渠道的执行路径
- 💻 **丰富客户端矩阵**：C# CLI、Rust TUI、Electron Desktop，终端到桌面全覆盖
- 🛠️ **现代 Agent 基础建设**：文件、Shell、Web、SubAgent 工具，MCP、Skills、Hooks、斜杠命令全面支持
- 🖥️ **ACP 编辑器集成**：Unity、JetBrains IDE、Obsidian 原生接入
- 🔗 **服务端能力**：OpenAI 兼容 API 与 AG-UI 协议，将 DotCraft 作为 Agent 后端
- 🌐 **可扩展渠道接入**：提供 Python 与 TypeScript SDK，快速接入 Telegram、微信、Discord 等社交平台
- 👥 **自动化管线**：支持本地任务与 GitHub Issue/PR 编排
- ⚗️ **上下文友好**：自动压缩、记忆整合、MCP 工具延迟加载，长会话更高效
...
TUI 是基于 Ratatui 构建的终端界面，通过 Wire Protocol 连接 AppServer。
...
使用 `dotcraft app-server` 显式启动。

### Desktop 桌面应用

DotCraft Desktop 是一个基于 Electron + React 的桌面应用，作为 AppServer 的图形化客户端工作。它通过 Wire Protocol 消费服务端暴露的会话、审批、计划、定时任务与自动化能力，提供三栏式工作区、多会话管理、实时流式对话，以及内联 Diff 查看与一键回滚。
...
Python 与 TypeScript SDK（`DotCraftClient`、`ChannelAdapter`）便于接入外部渠道。各平台适配器可用原生 UI 呈现审批流程，整体适配也更灵活。
...
| Telegram（Python SDK） | QQ / 企业微信 | 微信（TypeScript SDK） |
|:---:|:---:|:---:|
...
DotCraft 的 Automations 使用统一的 `AutomationOrchestrator` 编排自动化任务，当前包含 `Local` 与 `GitHub` 两类来源。`Automations.Enabled` 默认是 `true`，而 `GitHubTracker.Enabled` 默认保持 `false`，需要你显式开启。裸运行 `dotcraft` 仍然进入 CLI；如果你想单独跑并发 Channel / Automations 宿主，请使用 `dotcraft gateway`。详见 [Automations 指南](./docs/automations_guide.md)。
...
- [TypeScript SDK](./sdk/typescript/README.md)：使用 `dotcraft-wire`（TypeScript）与微信参考示例构建外部适配器
