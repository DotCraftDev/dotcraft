...
DotCraft Desktop 是一个基于 Electron + React 的桌面应用，作为 AppServer 的图形化客户端工作。它通过 Wire Protocol 消费服务端暴露的会话、审批、计划、定时任务与自动化能力，提供三栏式工作区、多会话管理、实时流式对话，以及内联 Diff 查看与一键回滚。支持 Windows、macOS 和 Linux。
